    ...
    <div class="container">
        <div class="row">
            <div class="col-md-6 ms-auto text-md-end">
                ...
                <a href="{{ route('ausencia.index') }}" class="btn btn-purple-light">Ausencias</a>
                    
                    <button class="btn btn-success" {{ $asistencias->isEmpty() ? 'disabled' : '' }}>
                    <a href="{{ route('export.excel') }}" style="color: white; text-decoration: none;">
                    <i class="fa-solid fa-file-excel fa-lg" style="margin-right: 5px;"></i> Generar Reporte
                    </a>
                    </button>
                    <div>
                        <form id="depurarForm" action="{{ route('asistencia.depurar') }}" method="post">
                            @csrf
                            <button type="submit" form="depurarForm" onclick="return confirm('¿Estás seguro de borrar todos los registros de asistencias?');" class="btn btn-danger" style="margin-left: 5px;" {{ $asistencias->isEmpty() ? 'disabled' : '' }}>
                                <i class="fa-solid fa-trash fa-lg" style="margin-right: 5px;"></i>
                            </button>
